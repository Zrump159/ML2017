...
class BiasModel(Sequential):
    ...
    def __init__(self, n_users, m_items, k_factors, **kwargs):
        ...
        Q.add(Reshape((k_factors,)))
        PB = Sequential()
        PB.add(Embedding(n_users, 1, input_length=1))
        PB.add(Reshape((1,)))
        QB = Sequential()
        QB.add(Embedding(n_users, 1, input_length=1))
        QB.add(Reshape((1,)))
        super(BiasModel, self).__init__(**kwargs)
        D = Sequential()
        D.add( Merge([P, Q], mode='dot',dot_axes=1) )
        self.add( Merge([D, QB, PB], mode='sum'))
