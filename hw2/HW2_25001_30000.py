...
csvfile.close()
#/////////////////////////////////////////////////////////
with open('30000_weight.csv') as csvfile :
    reader = csv.reader(csvfile, delimiter=',')
    for row in reader :
        weight = row[0:105]
        bias = row[105]
...
for i in range(itr) :
    ...
    print('Error : ', error)
    print('Loss : ',Loss_fuc)
    g_weight += 2.0 * labda * weight
    w_adagrad += (g_weight**2)
    b_adagrad += (g_bias**2)
    weight += -(1./w_adagrad**0.5)*(Lrnrate*g_weight)
    bias += -(1./b_adagrad**0.5)*(Lrnrate*g_bias)

#//////////////////////////////////////////////////////////
temp = [0.0]*106
with open("30000_weight.csv", 'w' ) as outfile :
